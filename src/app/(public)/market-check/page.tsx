...
import { MapPin, Search, ArrowRight, Users, CheckCircle2, AlertTriangle, Loader2, Shield } from "lucide-react";
import Link from "next/link";
import { getMarketData, getLocationSuggestions } from "./actions";
    ...
    <div className="min-h-dvh bg-neuro-navy flex flex-col">
      ...
      <div className="px-6 pt-12 pb-8 text-center">
        <div className="flex items-center gap-2 justify-center mb-4">
          ...
          <span className="text-xs font-black uppercase tracking-[0.3em] text-neuro-orange">Market Check</span>
        </div>
        <h1 className="text-3xl font-heading font-black text-white mb-3">
          Is Your City Covered?
        </h1>
        <p className="text-gray-400 text-sm max-w-xs mx-auto">
          See how many nervous system chiropractors serve your area — and whether there&apos;s room for you.
        </p>
      </div>
        ...
        <div className="max-w-sm mx-auto space-y-3">
          ...
          <div className="relative">
            <input
              ...
              className="w-full px-4 py-4 bg-white/5 border-2 border-white/10 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-neuro-orange text-sm"
            />
            <AnimatePresence>
              {showSuggestions && (
          ...
          <select
            ...
            className="w-full px-4 py-4 bg-white/5 border-2 border-white/10 rounded-xl text-white focus:outline-none focus:border-neuro-orange text-sm appearance-none"
          >
            <option value="" className="bg-neuro-navy">Select state</option>
            {US_STATES.map(s => (
              <option key={s} value={s} className="bg-neuro-navy">{s}</option>
            ))}
          </select>

          <button onClick={handleSearch} disabled={!city || !state || loading}
            className="w-full py-4 bg-neuro-orange text-white font-bold rounded-xl flex items-center justify-center gap-2 hover:bg-neuro-orange/90 transition-colors disabled:opacity-30">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="px-6 pb-8 flex-1">
            <div className="max-w-sm mx-auto space-y-4">
              ...
              <div className={`rounded-2xl p-6 text-center border-2 ${
                ...
                result.cityCount <= 3 ? 'bg-neuro-orange/10 border-neuro-orange/30' :
                  ...
                  <>
                    ...
                    {result.cityCount <= 3 && (
                      <div className="flex items-center gap-1.5 justify-center mt-3">
                        <AlertTriangle className="w-3.5 h-3.5 text-neuro-orange" />
                        <p className="text-xs text-neuro-orange font-bold">Limited coverage — claim your spot</p>
                      </div>
                    )}
                  </>
                )}
              ...
              {/* National */}
              <div className="bg-white/5 border border-white/10 rounded-2xl p-5 text-center">
                <p className="text-3xl font-black text-neuro-orange mb-1">{result.totalCount}+</p>
                <p className="text-sm text-gray-400">verified doctors nationwide</p>
              </div>

              {/* Urgency Message */}
              <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                <div className="flex items-start gap-3">
                  <Shield className="w-5 h-5 text-neuro-orange shrink-0 mt-0.5" />
                  <div>
                    <p className="text-sm font-bold text-white mb-1">
                      {result.cityCount === 0 ? `${result.cityName} is unclaimed.` :
                    ...
                    <p className="text-xs text-gray-400">
                      {result.cityCount === 0 ? "Be the first nervous system chiropractor patients find when they search your city." :
                       result.cityCount <= 3 ? "There's room for you. Get listed before your competition does." :
              ...
              {/* CTA */}
              <Link
                href={`/get-started?role=doctor&city=${encodeURIComponent(result.cityName)}&state=${encodeURIComponent(result.stateName)}`}
                className="w-full py-4 bg-neuro-orange text-white font-bold rounded-xl flex items-center justify-center gap-2 hover:bg-neuro-orange/90 transition-colors shadow-lg shadow-neuro-orange/20"
              >
                Claim Your Spot — Free <ArrowRight className="w-5 h-5" />
              </Link>
              <p className="text-[10px] text-gray-600 text-center">Free forever. No credit card required.</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      {!result && (
        <div className="mt-auto px-6 pb-8 text-center">
          ...
          <p className="text-white font-heading font-bold text-sm">NEURO<span className="text-neuro-orange">CHIRO</span></p>
        </div>
      )}
    </div>
  );
