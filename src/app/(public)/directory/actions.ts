...
export async function getDoctors(options: {
  ...
  try {
    ...
    const selectFields = 'id, first_name, last_name, clinic_name, slug, city, state, country, verification_status, membership_tier, is_founding_member, latitude, longitude, bio, specialties, region_code, phone, photo_url, created_at';
    ...
    // Priority sort: Founding → Boosted → Tier (Pro > Free)
    const tierPriority: Record<string, number> = { pro: 1, growth: 1, free: 2, basic: 2, starter: 2 };
    if (data) {
...
export async function getDoctorBySlug(slug: string) {
  ...
  const selectFields = 'id, first_name, last_name, clinic_name, slug, city, state, country, verification_status, membership_tier, is_founding_member, address, latitude, longitude, bio, specialties, region_code, email, phone, website_url, instagram_url, facebook_url, user_id, photo_url, video_url, seo_keywords, profile_views, patient_leads, banner_url, highlights, conditions_treated, education, languages, hours, accepted_payment, faq, gallery_images, booking_url, first_visit_info, parking_info, amenities, offers_telehealth, accepts_walkins, accepting_new_patients, years_in_practice, insurance_networks, team_members, certifications, created_at, first_visit_price, payment_model, files_insurance, payment_plans, free_consultation, has_evening_hours, has_weekend_hours, short_intro, philosophy, chiropractic_school, degree, graduation_year, post_doctoral_training, google_reviews_url';
