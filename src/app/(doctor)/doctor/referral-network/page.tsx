    ...
    <div className="p-4 md:p-10 max-w-5xl mx-auto space-y-6">
      <div>
        <div className="flex items-center gap-2 mb-1">
          ...
          <h1 className="text-2xl md:text-3xl font-bold text-neuro-navy tracking-tight">Referral Network</h1>
        </div>
        <p className="text-gray-500 text-sm">Track your doctor-to-doctor referral relationships.</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-2xl border border-gray-100 p-5 text-center">
          ...
          <p className="text-2xl font-black text-neuro-navy">{stats.totalSent}</p>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5 text-center">
          ...
          <p className="text-2xl font-black text-neuro-navy">{stats.totalReceived}</p>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5 text-center">
          ...
          <p className="text-2xl font-black text-neuro-navy">{stats.uniquePartners}</p>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5 text-center">
          ...
          <p className="text-2xl font-black text-neuro-navy">{stats.thisMonth}</p>
      ...
      {stats.topPartner && (
        <div className="bg-gradient-to-r from-neuro-orange/5 to-white rounded-2xl border border-neuro-orange/20 p-5 flex items-center justify-between">
          <div>
            ...
            <p className="text-lg font-bold text-neuro-navy">{stats.topPartner.name}</p>
          </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-bold text-gray-400 uppercase tracking-widest">Your Network</h2>
        ...
        {network.length === 0 ? (
          <div className="bg-white rounded-2xl border border-gray-100 p-8 text-center">
            ...
            <p className="text-gray-500 text-sm mb-4">No referral connections yet. Start by referring a patient to another doctor on the platform.</p>
              ...
              <div key={i} className="bg-white rounded-xl border border-gray-100 p-5 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  {doc.photoUrl ? (
                    ...
                    <div className="w-12 h-12 rounded-xl bg-neuro-navy/5 flex items-center justify-center text-neuro-navy font-bold">
                      {doc.name.split(' ').map((n: string) => n[0]).join('').slice(0, 2)}
                    </div>
                  )}
                  <div>
                    <p className="font-bold text-neuro-navy">{doc.name}</p>
                    {doc.clinicName && <p className="text-xs text-gray-500">{doc.clinicName}</p>}
                    {doc.city && <p className="text-xs text-gray-400 flex items-center gap-1 mt-0.5"><MapPin className="w-3 h-3" />{doc.city}{doc.state ? `, ${doc.state}` : ''}</p>}
                  </div>
                </div>
                <div className="flex items-center gap-6 shrink-0">
                  ...
                  {doc.slug && (
                    <Link href={`/directory/${doc.slug}`} className="px-3 py-1.5 bg-gray-50 text-gray-500 text-xs font-bold rounded-lg hover:bg-neuro-orange/5 hover:text-neuro-orange transition-colors">
                      Profile
                    </Link>
                  )}
                </div>
